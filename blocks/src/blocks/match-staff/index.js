...
class BlockEdit extends Component {
  ...
  constructor(props) {
    ...
    if(typeof this.props.attributes.columns === 'undefined'){
      this.props.setAttributes({columns: ['staff', 'citizenship']});
    }
  ...
  render() {
    ...
    const teamSlotData = {
      action: 'daextsoenl_get_team_slot_list',
      attributeName: 'teamSlot',
      title: __('Team', 'soccer-engine-lite'),
    };

    const columnsData = {
      action: 'daextsoenl_get_columns_match_staff',
    ...
    return [
      <div className="daextsoenl-block-image">{__('Match Staff', 'soccer-engine-lite')}</div>,
 ...
 */
registerBlockType('daextsoenl/match-staff', {
  title: __('Match Staff', 'soccer-engine-lite'),
  icon: BallIcon,
  category: 'daextsoenl-soccer-engine',
  keywords: [
    __('match staff', 'soccer-engine-lite'),
