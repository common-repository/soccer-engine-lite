...
class BlockEdit extends Component {
  ...
  constructor(props) {
    ...
    if(typeof this.props.attributes.playerAwardTypeId === 'undefined'){
      this.props.setAttributes({playerAwardTypeId: "0"});
    }
    if(typeof this.props.attributes.columns === 'undefined'){
      this.props.setAttributes({columns: ['player', 'age', 'citizenship', 'player_award_type', 'assignment_date']});
    }
  ...
  render() {
    ...
    const playerAwardTypeIdData = {
      action: 'daextsoenl_get_player_award_type_list',
      attributeName: 'playerAwardTypeId',
      title: __('Player Award Type', 'soccer-engine-lite'),
    };

    const columnsData = {
      action: 'daextsoenl_get_columns_player_awards',
    ...
    return [
      <div className="daextsoenl-block-image">{__('Player Awards', 'soccer-engine-lite')}</div>,
      <InspectorControls key="inspector">
        <GenericReactSelect data={playerAwardTypeIdData} attributes={this.props.attributes} setAttributes={this.props.setAttributes} />
 ...
 */
registerBlockType('daextsoenl/player-awards', {
  title: __('Player Awards', 'soccer-engine-lite'),
  icon: BallIcon,
  category: 'daextsoenl-soccer-engine',
  keywords: [
    __('player', 'soccer-engine-lite'),
    __('awards', 'soccer-engine-lite'),
