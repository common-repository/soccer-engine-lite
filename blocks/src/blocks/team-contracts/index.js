...
class BlockEdit extends Component {
  ...
  constructor(props) {
    ...
    if(typeof this.props.attributes.teamContractTypeId === 'undefined'){
      this.props.setAttributes({teamContractTypeId: "0"});
    }
    if(typeof this.props.attributes.columns === 'undefined'){
      this.props.setAttributes({columns: ['player', 'age', 'citizenship', 'start_date', 'end_date', 'team', 'team_contract_type', 'salary', 'market_value', 'agency']});
    }
  ...
  render() {
    ...
    const teamContractTypeIdData = {
      action: 'daextsoenl_get_team_contract_type_list',
      attributeName: 'teamContractTypeId',
      title: __('Team Contract Type', 'soccer-engine-lite'),
    };

    const columnsData = {
      action: 'daextsoenl_get_columns_team_contracts',
    ...
    return [
      <div className="daextsoenl-block-image">{__('Team Contracts', 'soccer-engine-lite')}</div>,
      <InspectorControls key="inspector">
        <GenericReactSelect data={teamIdData} attributes={this.props.attributes} setAttributes={this.props.setAttributes} />
        <GenericReactSelect data={teamContractTypeIdData} attributes={this.props.attributes} setAttributes={this.props.setAttributes} />
 ...
 */
registerBlockType('daextsoenl/team-contracts', {
  title: __('Team Contracts', 'soccer-engine-lite'),
  icon: BallIcon,
  category: 'daextsoenl-soccer-engine',
  keywords: [
    __('team', 'soccer-engine-lite'),
